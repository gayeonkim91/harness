...
- guided repo profile에 `verification_toolchain.configured=true`가 있으면 `/wf-start`는 repo 최초 세팅 때 선택한 build/test/gate 도구를 `plan.md` Verification 초기 계약에 우선 반영한다
...
- PR8부터 verification 보조 core는 영향 범위별 gate selector, Gradle result inspector, test-report skill bridge schema를 제공한다. `/wf-verify` runtime은 stack trace 원문 기록을 차단하고 test-report 미경유 결과를 lint warning으로 남긴다
