...
def test_runtime_cli_rejects_string_boolean_review_payload(monkeypatch, tmp_path: Path) -> None:
    _, task_root, _ = _workspace_with_review_state(tmp_path)
    payload = {
        "task_root": str(task_root),
        "workspace_root": str(REPO_ROOT),
    ...
    try:
        ...
        assert "Boolean field must be a JSON boolean" in str(exc)
    else:
        raise AssertionError("String boolean review payload should be rejected.")
